...
| 周报 | 按周期生成、AI 生成（注入历史风格/项目全景/里程碑/上周计划/写作规则）、Markdown 与结构化双模编辑、叙述板块与派生列逐项可配、版本历史 |
...
| 13 | 周报结构扩展：本周概览/关键成果/问题与风险三个叙述板块，人天·占比·优先级·交付物四个可选列，范文机制下放到周报，板块与列逐项开关；解析器改列名定位、`buildMarkdown` 改小节级重建（#22） |
| 14 | 结构化视图补上新增板块与派生列，按原文小节顺序排版（#23） |
| 15 | 周报注入项目全景（含本周无投入的项目）、本周里程碑、项目档案与上周计划；富文本粘贴自动转 Markdown 表格（#24） |
| 16 | 修复范文结构（「一、二、」标题、无 `##`）下结构化视图一片空白：开头块纳入渲染并可编辑（#25） |
| 17 | 结构化视图支持任意表格：文本块拆成「段落 / 表格」交替，非标准表格给网格控件逐格编辑（#26） |
...
周报正文的唯一真相是 Markdown 字符串，结构化视图是它的编辑投影。三条约束：

- **小节级重建**：回写时只替换两张标准表所在的小节，其余小节按原顺序原样保留。否则叙述板块与范文产出的自定义小节会在模式切换中被抹掉
- **开头块也要渲染**：第一个 `##` 之前的内容（问候语，或范文用「一、二、」做标题时的整篇正文）纳入可编辑块，否则范文格式下结构化视图一片空白
- **表格按列名定位**：列是可配置的（人天/占比/优先级/交付物随开关增减），固定下标一加列就整表错位；长别名优先匹配，否则「项目进度」会被「项目」抢走

两张标准表保留富编辑（工时输入联动工作记录、进度下拉与汇总页双向同步）；其余表格走通用网格控件，只做逐格文本编辑。
...
- 富文本粘贴只认真表格（`<table>`）。部分 OA/邮件用带边框的 `<div>` 拼出表格外观，转换器会放行走默认粘贴——绕法是先粘进 Word/飞书再复制
- 合并单元格（colspan/rowspan）无法在 Markdown 中表达，粘贴与网格编辑均按最宽行补空格
- 未做「项目当前状态」一句话字段：范文总览表的状态列因此只能由 AI 推断，本周无投入的项目尤其薄弱
- 未做「问题跟踪」台账：范文「问题与反馈」里跨周累积的应对措施无处维护，现有「问题与风险」只能从本周工作内容提炼
