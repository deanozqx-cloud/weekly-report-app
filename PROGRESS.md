...
- **项目进度可自定义选项**：周报编辑器中「项目进度」改为 `EditableSelect`，底部可输入自定义选项并保存到 `settings.progressOptions`
- **项目整体状态**：汇总页每个项目卡片右侧新增「设置状态」下拉框，独立选项列表（`settings.statusOptions`），状态保存到 `settings.projectStatuses`，与项目进度完全隔离
- **项目关键词搜索筛选**：明细页和汇总页均新增 `SearchableProjectSelect`，输入关键词实时过滤项目，支持精确选中
- **数据管理**：设置页底部新增「清空工作记录 / 清空周报 / 清空全部数据」按钮，操作带二次确认
